...
def predict_ml_model(df, model, model_name, periods=12):
    ...
    try:
        ...
        from features import build_features_ml
        ...
        # For demonstration, use simple trend-based prediction
        # In real implementation, you'd need proper feature engineering
        recent_values = df.iloc[-12:]['Value'].values
...
def main():
    ...
    available_agencies = kpi_data['Agency'].unique()
    ...
    # Get filtered data
    filtered_data = df[(df['KPI'] == selected_kpi) & (df['Agency'] == selected_agency)].copy()
    ...
    if not predictions.empty:
        ...
        with col1:
            st.metric(
                "Next Month Forecast", 
                f"{predictions.iloc[0]['Prediction']:,.0f}",
                f"{((predictions.iloc[0]['Prediction'] / filtered_data.iloc[-1]['Value']) - 1) * 100:+.1f}%"
            )
        ...
        with col2:
            ...
            st.metric(
                "Average Forecast", 
                f"{avg_forecast:,.0f}",
                f"{((avg_forecast / filtered_data['Value'].mean()) - 1) * 100:+.1f}%"
            )
